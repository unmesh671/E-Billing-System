...
public class PayBill extends JFrame implements ActionListener{
    ...
    PayBill(String meter) {
        ...
        try {
            ...
            ResultSet rs = conn.state.executeQuery("select * from customer where meter_no = '"+meter+"'");
            while(rs.next()) {
                meternumber.setText(meter);
                labelname.setText(rs.getString("name"));
            }
            
            rs = conn.state.executeQuery("select * from bill where meter_no = '"+meter+"' AND month = 'January'");
            while(rs.next()) {
        ...
        cmonth.addItemListener(new ItemListener(){
            ...
            public void itemStateChanged(ItemEvent ae) {
                try {
                    DatabaseConnection conn = new DatabaseConnection();
                    ResultSet rs = conn.state.executeQuery("select * from bill where meter_no = '"+meter+"' AND month = '"+cmonth.getSelectedItem()+"'");
                    while(rs.next()) {
                        labelunits.setText(rs.getString("units"));
                        labeltotalbill.setText(rs.getString("totalbill"));
                        labelstatus.setText(rs.getString("status"));
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == pay) {
            try {
                DatabaseConnection conn = new DatabaseConnection();
                conn.state.executeUpdate("update bill set status = 'Paid' where meter_no = '"+meter+"' AND month='"+cmonth.getSelectedItem()+"'");
            } catch (Exception e) {
                e.printStackTrace();
            }
            setVisible(false);
